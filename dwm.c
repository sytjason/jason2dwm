...
enum { NetSupported, NetWMName, NetWMIcon, NetWMState, NetWMCheck, NetSystemTray, NetSystemTrayOP, NetSystemTrayOrientation, NetSystemTrayOrientationHorz, NetWMFullscreen, NetActiveWindow, NetWMWindowType, NetWMWindowTypeDialog, NetClientList, NetLast }; /* EWMH atoms */
...
drawbar(Monitor *m)
{
	...
	if ((w = m->ww - sw - stw - x) > bh) {
        ...
        if (m->sel == c)
				...
				drw_text(drw, x, 0, tabw, bh, lrpad / 2 + (c->icon ? c->icon->width + ICONSPACING : 0), c->name, 0);
